...
[y, Fs] = wavread('i-think-that-this-is.wav');
subplot(311);
x_n = y(:, 1:end/2);
plot(x_n)
%sound(x_n,Fs);
...
xlim([-6e3 6e3])  
Vpp = ceil(max([max(x_n) abs(min(x_n))]));
k = 8; % qtd de bits do quantizador
Rb = 1e3;
...
% Fim da quantizacao
subplot(313)
filtro_NRZ = ones(1, 1e3); % filtro
%tx = filter(filtro_NRZ, 1, x_bin_linha);
info = upsample(x_bin_linha, 1e3);
%tx = filter(filtro_NRZ, 1, info);
tx = conv(filtro_NRZ, info);
plot(tx);
ylim([-0.5 8.5])
xlim([0 10000])
%% Recuperando o Sinal
